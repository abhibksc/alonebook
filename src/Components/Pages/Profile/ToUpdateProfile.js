...
import UpdateProfile, { UpdateProfileImage } from '../../../CrudOperations/Update&Edit';
import { GetProfileImage, userIdChecker } from '../../../CrudOperations/GetOperation';
...
const Profile = () => {
  ...
  // using UserNameToggle , sending the updated userName with (fetching ProfileImge from the database) to database
    ...
    console.log(UserNameToggle);
      ...
      const fun = async () => {
          ...
          if (ProfileImg && UserName && userId) {
            const data = await UpdateProfile({Name : profileName, UserName, userId, ragistered, imgFile: ProfileImg });
            dispatch(update({
  ...
  // using toggle , sending the updated image with existing UserName to database

  useEffect(() => {
    const UpdateImg = async () => {
      if (toggle) {
        if (imgFile !== null && imgFile && userId && UserName && ragistered) {
          const data = await UpdateProfileImage({Name : profileName, userId, UserName, imgFile, ragistered })
          if (data.error) {
  ...
  return (
    <div className="div_14 relative z-30">
      {/* for Laptop */}
      <CloseOutlinedIcon className=' relative  md:top-9 md:left-[1300px]  top-5' onClick={(e) => navigate("/Dashboard/Home")} />
          ...
          <div className="flex justify-around">
            ...
            <div className="mt-20 flex flex-col">
              ...
              <img
                ...
                className={`${Profileimage ? "imge2 object-cover md:w-[350px] rounded-full h-[350px] shadow-lg shadow-green-300" : "imge2 object-cover md:w-[350px] rounded-full h-[350px] shadow-lg  bg-gray-400 shadow-green-300"}`}
                onClick={(() => fileInputRef.current.click())} />
              {userName && <h1 className="mx-auto text-center mt-6 text-black">{UserName}</h1>}
            </div>
          </div>
        </form>
      </div>
      ...
      {/* for Mobile */}
      <form onSubmit={handleFormSubmit} className=" md:hidden  z-3 h-screen w-screen  flex  flex-col  gap-10  bg-white ">
        <div className="  flex flex-col text-black pt-10    gap-2 ">
          ...
          <img src={`${Profile_Img_URL}${userId}%2FProfile%2FimgFile?alt=media&token=${Profileimage}`}

            className={`${Profileimage ? "mx-auto w-40 object-cover rounded-full h-40 shadow-lg shadow-green-300 " : "mx-auto w-40 object-cover rounded-full h-40 shadow-lg    bg-gray-400 shadow-green-300"}`}
        ...
        <button className="border-2 rounded-lg text-black  mx-auto w-28 h-10  hover:text-white hover:bg-green-500" type="submit">Update</button>
